...
import com.pascalschumann.jobshopschedulermicroservice.scheduler.IJobShopScheduler;
 ...
 */
@Component
public class JobShopScheduler implements IJobShopScheduler {

    /*
     * private void CorrectIdleStartTimesOfMachines(IEnumerable<Operation> operations,
     * Dictionary<Id, List<Resource>> resourcesByResourceSkillId) { for (var operation in
     * operations) { for (var resource in
     * resourcesByResourceSkillId[operation.GetResourceSkillId()]) { if
     * (resource.GetValue().Id.Equals(operation.GetValue().ResourceId) &&
    ...
    public void ScheduleWithGifflerThompsonAsZaepfel(final IPriorityRule priorityRule,
        ...
        /*
         * // set correct idleStartTimes in resources from operations of last cycle(s) // TODO:
         * implement this CorrectIdleStartTimesOfMachines(dbTransactionData.OperationGetAll(),
         * machinesByMachineGroup);
         */

        /*
         * S: Menge der aktuell einplanbaren Arbeitsvorgänge a: Menge der technologisch an erster
         * Stelle eines Fertigungsauftrags stehenden Arbeitsvorgänge N(o): Menge der technologisch
         * direkt nachfolgenden Arbeitsoperationen von Arbeitsoperation o M(o): Maschine auf der die
         * Arbeitsoperation o durchgeführt wird K: Konfliktmenge (die auf einer bestimmten Maschine
         * gleichzeitig einplanbaren Arbeitsvorgänge) p(o): Bearbeitungszeit von Arbeitsoperation o
         * (=Duration) t(o): Startzeit der Operation o (=Start) d(o): Fertigstellungszeitpunkt von
         * Arbeitsoperation o (=End) d_min: Minimum der Fertigstellungszeitpunkte o_min: Operation
         * mit minimalem Fertigstellungszeitpunkt o1: beliebige Operation aus K (o_dach bei Zäpfel)
         */
        ...
        while (S != null && S.Any()) {
            ...
            for (final Operation o : S) {
                if (o.getMachineGroupId().equals(o_min.getMachineGroupId())
                                && o.getStart() < d_min) {
                    K.Push(o);
                }
            }
            ...
            if (K.Any()) {
                // Entnehme Operation mit höchster Prio (o1) aus K und plane auf nächster freier
                // Resource ein

                final List<Operation> allO1 = new LinkedList<>();

                machinesByMachineGroup.get(o_min.getMachineGroupId())
                                .sort(Comparator.comparing(Machine::getIdleStart));
                for (final Machine machine : machinesByMachineGroup
                                .get(o_min.getMachineGroupId())) {
                    if (K.Any() == false) {
                        break;
                    }

                    Operation o1 = null;
                    o1 = priorityRule.GetHighestPriorityOperation(machine.getIdleStart(),
                                    K.GetAll());
                    if (o1 == null) {
                    ...
                    if (dueTimeOfOperationMaterial != null
                                    && dueTimeOfOperationMaterial > o1.getStart()) {
                ...
                for (final Operation o1 : allO1) {
                    ...
                    final INodes allPredecessorsRecursive =
